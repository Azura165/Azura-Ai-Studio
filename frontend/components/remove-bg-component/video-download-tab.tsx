...
export function VideoDownloadTab() {
  ...
  const saveToHistory = (info: VideoInfo, url: string) => {
    ...
    const newHistory = [newItem, ...history.filter((h) => h.url !== url)].slice(
      0,
      10,
    );
  ...
  const handlePaste = async () => {
    try {
      ...
      setUrl(text);
      toast.info("Link ditempel dari clipboard");
    } catch (err) {
  ...
  const handleGetInfo = async (overrideUrl?: string) => {
    ...
    setVideoInfo(null);
    const toastId = toast.loading("Sedang mencari video...");
    ...
    try {
      ...
      if (!res.ok) throw new Error("Video tidak ditemukan atau server sibuk.");
  ...
  const handleHistoryClick = (itemUrl: string) => {
    ...
    }, 50); // Delay dikurangi jadi 50ms biar makin ngebut
  };
  ...
  const handleDownload = async (formatId: string, resolution: string) => {
    ...
    try {
      ...
      a.download = `RaditStudio-${resolution}.${ext}`;
  ...
  return (
    <div className="relative space-y-8 max-w-3xl mx-auto px-1">
      ...
      <div className="relative flex flex-col md:flex-row items-center justify-center min-h-[40px] gap-4 mb-2">
        <div className="flex items-center gap-5 text-muted-foreground/60 md:absolute md:left-1/2 md:-translate-x-1/2 z-0">
          <Youtube className="size-5 md:size-6 hover:text-red-500 transition-all hover:scale-110 cursor-pointer" />
          <Instagram className="size-5 md:size-6 hover:text-pink-500 transition-all hover:scale-110 cursor-pointer" />
          <div className="flex items-center justify-center size-5 md:size-6 font-bold text-lg hover:text-black dark:hover:text-white transition-all hover:scale-110 cursor-default">
            T
          </div>
          <XLogo className="size-4 md:size-5 hover:text-foreground transition-all hover:scale-110 cursor-pointer" />
          <Facebook className="size-5 md:size-6 hover:text-blue-600 transition-all hover:scale-110 cursor-pointer" />
        </div>
      ...
      <div className="text-center pb-2">
        <p className="text-sm text-muted-foreground">
          Paste link video dari platform favoritmu di bawah ini.
        </p>
      </div>
      ...
      {/* MODAL HISTORY (LIGHTWEIGHT VERSION) */}
      <AnimatePresence>
        {showHistoryModal && (
          // HAPUS backdrop-blur-sm DI SINI 👇 (Ganti jadi bg-black/80 yang solid transparan)
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 animate-in fade-in duration-200">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              transition={{ duration: 0.15 }} // Animasi lebih cepat
                  ...
                  history.map((item, idx) => (
                    ...
                    >
                      <div className="relative w-20 aspect-video rounded-lg overflow-hidden bg-muted shrink-0">
                        {/* OPTIMASI GAMBAR: sizes="80px" biar download kecil */}
                        <Image
                          src={item.thumbnail}
                          alt="thumb"
                          fill
                      ...
                      <ExternalLink className="size-4 text-muted-foreground opacity-0 group-hover:opacity-100 self-center" />
                    </div>
                  ))
                )}
              </div>
      ...
      <AnimatePresence mode="wait">
        {videoInfo && (
          <motion.div
            ...
            className="rounded-3xl border bg-card overflow-hidden shadow-2xl shadow-indigo-500/10 ring-1 ring-border/50"
          >
            ...
            <div className="p-6 md:p-8 bg-card relative">
              ...
              {isDownloading && (
                <div className="absolute inset-0 bg-background/60 backdrop-blur-sm z-20 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300 rounded-b-3xl">
                  <div className="bg-background border p-8 rounded-3xl shadow-2xl flex flex-col items-center max-w-sm ring-1 ring-border">
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      ...
                      Mohon tunggu, jangan tutup tab ini.
                    </p>
                  </div>
                </div>
              )}
                ...
                {videoInfo.formats.map((format, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "group relative flex flex-col p-4 rounded-2xl border-2 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg",
                        isMp3
